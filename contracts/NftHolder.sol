...
contract HoldingNFT is IERC721Receiver {
    ...
    event DeleteManager(address _manager);
    ...
    function claimToOwner(
        ...
        if (found) {
            Winner memory nft = winners[index];
            deleteWinnerAddress(nft.id, nft.winnerAddress);
        }
    }
    ...
    function getWinnersInfo(
        ...
        if (index > winners.length) {
            Winner[] memory nullResult = new Winner[](1);
            return nullResult;
        }

        if (lastIndex > winners.length) {
            lastIndex = winners.length;
        }
        uint256 resultSize = lastIndex - index;

        // Initialize the results array
        Winner[] memory results = new Winner[](resultSize);

        for (uint256 i = 0; i < resultSize; i++) {
            results[i] = winners[index + i];
        }
