...
contract Lottery is HoldingNFT {
    ...
    /**
     * @dev This function returns the player list paginated.
     * @param _page The page number to fetch from the winners list. If it's less than or equal to zero, then it will be set to one by default.
     * @return playersList An array of NFT structs representing the winners in a specific page. It returns at most 5 items per page.
     */

    function getParticipents(
        uint256 _page
    ) public view returns (address[] memory playersList) {
        uint256 limit = 20;
        if (_page <= 0) {
            _page = 1;
        }
        uint256 index = limit * (_page - 1);
        uint256 lastIndex = index + limit;

        if (index > players.length) {
            address[] memory nullResult = new address[](1);
            return nullResult;
        }
        ...
        uint256 resultSize = lastIndex - index;

        // Initialize the results array
        address[] memory results = new address[](resultSize);

        for (uint256 i = 0; i < resultSize; i++) {
            results[i] = players[index + i];
        }
     ...
     * @dev This function returns the player list paginated.
     * @param _page The page number to fetch from the winners list. If it's less than or equal to zero, then it will be set to one by default.
     * @return participentTickets An array of NFT structs representing the winners in a specific page. It returns at most 5 items per page.
     */

    function getParticipentTickets(
        uint256 _page
    ) public view returns (address[] memory participentTickets) {
        uint256 limit = 20;
        if (_page <= 0) {
            _page = 1;
        }
        ...
        if (index > playerSelector.length) {
            address[] memory nullResult = new address[](1);
            return nullResult;
        }

        if (lastIndex > playerSelector.length) {
            lastIndex = playerSelector.length;
        }
        uint256 resultSize = lastIndex - index;
        // Initialize the results array
        address[] memory results = new address[](resultSize);
        ...
        for (uint256 i = 0; i < resultSize; i++) {
            results[i] = playerSelector[index + i];
        }
